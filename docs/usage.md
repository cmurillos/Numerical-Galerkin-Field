...
El recorrido actual es geometría, espacio admisible, base y campo autónomo G.
Esta guía documenta el [contrato D-013](design-contract.md#d-013--contrato-de-uso-con-espacio-admisible-explícito)
implementado en este checkout. La [guía de migración](migration.md) explica la
compatibilidad con 0.9.0, los tamaños y el traslado de coordenadas entre bases.
Los fragmentos ilustran operaciones sobre objetos ya preparados; los ejemplos
marcados como completos incluyen sus datos espaciales. En el recorrido con Space,
incluso una ecuación escalar conserva el eje de componente `(1,)`.

| Símbolo | Significado | Forma correspondiente |
|---|---|---|
| M, E | Número de vértices y simplejos máximos. | `vertices:[M,p]`, `simplices:[E,k+1]`. |
| k, p | Dimensión intrínseca y ambiente, respectivamente. | `1 <= k <= p`; un toro superficial tiene k=2 y p=3. |
| c | Componentes físicas del estado. | `Space(components=c)` y `value_shape=(c,)`. |
| N | Dimensión total de la base operacional. | `basis.dimension`; último eje de z y G(z). |
| Q | Puntos de evaluación o de cuadratura. | `points:[Q,p]`, `basis.evaluate(points):[Q,N,c]`. |
| S | Ejes de lote arbitrarios. | `z:[*S,N]`, reconstrucción `[*S,Q,c]`. |
...
```python
boundaries = {"wall": np.array([[0]])}  # un extremo del intervalo
```

o mediante un predicado aplicado a los baricentros de todas las caras exteriores:

```python
boundaries = {"left": lambda x: np.isclose(x[:, 0], 0.0)}
```
...
En dimensión intrínseca k, cada cara contiene k índices de vértices; el ejemplo
anterior usa k=1. En ese caso ds integra puntos con medida de conteo: cada extremo
tiene peso uno, aunque su medida de longitud sea cero.
...
Las etiquetas no tienen que formar una partición. Sumar integrales sobre regiones
superpuestas incluye las contribuciones de cada región; la selección no elimina
solapamientos entre etiquetas diferentes.
...
ejemplo del recorrido actual está en [acceptance_torus.py](../examples/acceptance_torus.py).
...
```python
G = GalerkinField(basis=basis, weak=weak)  # automática
G = GalerkinField(basis=basis, weak=weak, quadrature=10)  # fija
G = GalerkinField(basis=basis, weak=weak, quadrature=1e-8)  # adaptativa
```
...
## Descripción del espacio admisible
...
La base se construye con `V.basis(...)`, como se describe a continuación.
...
basis = V.basis("laplacian", size=4, degree=2)
```

La condición se impone antes de seleccionar los cuatro modos. El tamaño solicitado
debe caber en el espacio nodal libre. Las condiciones naturales se escriben en la
forma, con independencia de esta elección espectral.

### Restricción explícita de una familia candidata

`V.basis("custom", source=raw)` restringe y ortonormaliza la fuente en una llamada.
Para inspeccionar la familia intermedia se dispone también de `V.restrict`:
...
```python
basis = V.basis("custom", source=admissible)
G = GalerkinField(basis=basis, weak=weak)
```
...
## Periodicidad, media cero y fronteras fijas
...
| `"custom"` | Todo el espacio de la fuente después de restringirlo; `size` opcional comprueba la dimensión. | Las mismas combinaciones en representaciones nodales verificables. |
...
No hay cambios silenciosos de coordenadas al construir el campo:

```python
from ngfield import GalerkinField, grad, inner
...
Una base espectral se construye sobre el espacio admisible declarado:
...
```text
K_adm c_j = lambda_j M_adm c_j,
c_i^T M_adm c_j = delta_ij,
```
...
`M_adm` y `K_adm` son las matrices internas ya restringidas; no son argumentos que
deba aportar el usuario. La [tabla de familias](#bases-del-espacio-admisible) indica
los tamaños y las restricciones admitidas en cada caso.
...
`L2(Omega_h)`, sumado sobre componentes. `GalerkinField` rechaza una familia cruda y no permite reemplazar el
...
```python
from ngfield import ComponentBasis, FiniteElementBasis, TransformedBasis

raw = ComponentBasis(FiniteElementBasis(geometry, degree=2), components=V.components)
admissible = TransformedBasis(raw, transform)
basis = V.basis("custom", source=admissible)
G = GalerkinField(basis=basis, weak=weak)
```

El usuario proporciona `transform` de forma `[raw.dimension,N]`: sus columnas
expresan las nuevas funciones en la familia candidata. Si una matriz C expresa
ecuaciones `C c=0`, la transformación debe tener columnas en su núcleo; pasar C
directamente a `TransformedBasis` no impone esas ecuaciones. No existe un argumento
general `constraints=` en Space: las restricciones declarativas implementadas se
entregan mediante `restrictions=[...]`.
...
Para asignar cantidades distintas a cada componente usando una misma familia:

```python
W = Space(geometry=geometry, components=3)
basis = W.basis("laplacian", component_sizes=(12, 8, 5))
```

La dimensión total es 25. Cada cantidad debe caber en su espacio admisible.
Una composición de familias escalares diferentes puede adaptarse como fuente
`custom`. La siguiente construcción usa los constructores escalares de la interfaz
anterior para preparar esa fuente:

```python
from ngfield import GalerkinProblem, ProductBasis
...
La fuente entra mediante `basis = W.basis("custom", source=source)`.
...
## Varias componentes y compatibilidad tensorial

```python
W = Space(geometry=geometry, components=2)
basis = W.basis("polynomial", component_sizes=(10, 10))
...
Aquí `value_shape=(2,)`, con 20 coordenadas en total. En geometrías donde los monomios
ambientales sean dependientes, esta familia debe sustituirse por una independiente.
`Space` describe un vector de componentes escalares, no un valor tensorial de forma
arbitraria. La interfaz explícita `GalerkinProblem` conserva las formas tensoriales
anteriores; véase la [migración](migration.md#valores-tensoriales-y-bases-de-la-interfaz-original).
`inner` contrae todos los ejes del valor físico y deja libres lote, modo de prueba
y cuadratura.
...
```python
from ngfield import sin


def weak(u, v, dx, ds):
    x = dx.x
    normal = ds.normal
    volume = -(1 + sin(x[0]) ** 2) * inner(grad(u[0]), grad(v[0])) * dx
    boundary = -(2 + inner(normal, normal)) * u[0] * v[0] * ds("wall")
...
k_cell = Coefficient.cell(cell_values)

# un valor por vértice, interpolado linealmente
k_vertex = Coefficient.vertex(vertex_values)
```

Para estos coeficientes físicos escalares, `cell_values` tiene forma `[E]` y
`vertex_values` forma `[M]`, con la numeración de la geometría. No son estados
iniciales de forma `[E,c]` o `[M,c]`.
...
def weak(u, v, dx, ds):
    return -inner(K @ grad(u[0]), grad(v[0])) * dx
```
...
evalúa este campo y conserva autograd respecto de `z`. `G.solve` integra después la
EDO autónoma. La identidad de normas es exacta bajo ortonormalidad exacta; en el
cálculo numérico queda sujeta al error del Gram y de la cuadratura.
...
Para una componente, una discontinuidad alineada con las caras de la malla se
describe con un valor `[1]` por simplex. En el intervalo del ejemplo inicial,
la partición x<0.5 está alineada con la malla:

```python
centers = G.geometry.vertices[G.geometry.simplices].mean(axis=1)
u0 = Coefficient.cell((centers[:, :1] < 0.5).astype(float))
z0 = G.project(u0)
...
En aritmética e integración exactas, `G.project` produce la mejor aproximación en
`L2` dentro de `V_N`; el programa aproxima esos productos por cuadratura. El resultado tiene
...
```python
points = torch.tensor(
    G.geometry.vertices[G.geometry.simplices[0, :1]], dtype=G.dtype, device=G.device
)
cells = torch.tensor([0], dtype=torch.int64, device=G.device)
...
familia no ortonormal debe prepararse con `V.basis`, o con la ortonormalización
explícita de la interfaz anterior. La evaluación divide automáticamente los modos de prueba
...
La carga FEM no recupera Space ni sus declaraciones de admisibilidad. Para conservar
exactamente una base cargada, se utiliza un `GalerkinProblem` explícito, como muestra
la [guía de migración](migration.md#persistencia-sin-perder-las-coordenadas).

La API 0.1 basada en `Domain`, `FEMSpace`, `Problem` y `GalerkinBasis` se conserva por
compatibilidad. El recorrido actual es `Space`, `V.basis` y
`GalerkinField(basis=basis, weak=weak)`; las rutas explícitas siguen disponibles para
las bases existentes y las representaciones que no pasan por Space.
